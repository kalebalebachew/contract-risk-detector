  ...
  try {
    ...
    try {
      ...
      if (!Array.isArray(analysis) || analysis.length === 0) {
        analysis = [{ isContract: false, reason: 'Analysis returned empty', summary: responseText }];
      }
    } catch (parseError) {
      console.warn('JSON Parse Error:', parseError.message, 'Raw Text:', responseText);
        ...
        analysis = partialMatch.map((chunk) => {
          try {
            ...
            return parsed.clause
              ? parsed
              : { isContract: false, reason: 'Partial response parsed', summary: chunk };
          } catch {
            return {
              isContract: false,
              reason: 'Unable to parse API response as JSON',
              summary: responseText.slice(0, 400) + (responseText.length > 400 ? '...' : ''),
            };
          }
        });
      } else {
    ...
    if (err.message.includes('API_KEY_INVALID')) errorMessage = 'Invalid API key';
    else if (err.message.includes('timed out')) errorMessage = 'API request timed out';
